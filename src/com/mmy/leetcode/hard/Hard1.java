...
public class Hard1 {
  ...
  /**
   * 自己的解法，先将其变为有序，再取其中位数 时间复杂度为nums1.length+nums2.length 空间复杂度为nums1.length+nums2.length
   */
  ...
  /**
   * 官方答案 空间复杂度为1 优于我 https://leetcode.com/problems/median-of-two-sorted-arrays/solution/
   */
   ...
   *
   * 此方法使用一维数组，无法有效*表示的0个或多个情况下的成立 一开始想的是字符串与Pattern从左开始匹配，每一次匹配已经认为前面匹配为最优解，
    ...
    while (s_index < chars_s.length && p_index < chars_p.length) {
      ...
      if (p_index < chars_p.length && chars_p[p_index] == '*') {
        ...
        if (temp_s > -1 && temp_p > -1) {
          //为x匹配x*状况
          int temp = temp_s;
          while (s_index < chars_s.length && (chars_s[temp_s] == chars_s[s_index]
              || chars_p[temp_p] == '.')) {
            s_index++;
          }
          p_index++;
  ...
  /**
   * 使用DP2维数组解决 Boolean array[i][j]代表 s[i-1]与p[j-1]是否匹配成功 if p[j]==s[i] :
   * array[i+1][j+1]=array[i][j] if p[j]=='.'  ：array[i+1][j+1] = array[i][j] if p[j] =='*' : if
   * p[j-1]=!s[i] : array[i+1][j+1]=array[i+1][j-1] if p[j-1]==s[i]||p[j-1]=='.':
   * 匹配多次，array[i+1][j+1]=array[i][j+1] 匹配0次, array[i+1][j+1]=array[i+1][j-1]
  ...
  /**
   * invoke merge2Lists one by one cost too many time(230ms+)
   * consider use MERGE-SORT ALGORITHM
   * already change to MERGE-SORT ALGORITHM(12ms) almost decrease half merge times
   * @param lists
   * @return
   */
  public ListNode mergeKLists(ListNode[] lists) {
    ListNode result =partition(0,lists.length-1,lists);
    return result;
  }

  public ListNode partition(int left,int right,ListNode[] listNodes){
    if (left==right){
      return  listNodes[left];
    }
    if (left<right){
      int mid = (left+right)/2;
      ListNode leftNode =  partition(left, mid, listNodes);
      ListNode rightNode = partition(mid+1,right,listNodes);
